 ...
 */
@HomeViewType(ViewType=200)
public class HomeViewPagerHolder extends HomeViewHolder {
    ...
    public HomeViewPagerHolder(Activity activity, LayoutInflater inflater, ViewGroup viewGroup) {
    ...
    private void createPageIndicator() {
        ...
        if (StringUtils.equals(BuildConfig.BUILD_TYPE, Constant.BUILD_TYPE_PMPH)) {
            indicator.setFillColor(ContextCompat.getColor(mConText, R.color.orange_ff6a00));
        } else {
            indicator.setFillColor(ContextCompat.getColor(mConText, R.color.red));
        }
        indicator.setPageColor(ContextCompat.getColor(mConText, R.color.gray_c0c1c3));
    ...
    private class MyAdapter extends PagerAdapter {
        ...
        public MyAdapter() {
            this.mListViews = new SparseArray<>();//构造方法，参数是我们的页卡，这样比较方便。
        }
        ...
        public Object instantiateItem(ViewGroup container, int position) {    //这个方法用来实例化页卡
           ImageView imageView = mListViews.get(position);
            ...
            setImageViewImg(items.get(position).getImgUrl(), imageView, mAQuery, 640);
